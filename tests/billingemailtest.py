...
class BillingEmail(unittest.TestCase, BaseTest):
    """ For testing all tests within billing email testcases
    Calls to all the actions in each pages are done here
    """
    def __init__(self):
        self.row = 11
        self.field_name = 'billing_email'
    ...
    def test_field_label(self, logger):
        self.field_label('billing_email_field_label', logger, section_type=self.section_type, action_type=self.action_type, row=self.row, field_name=self.field_name)

        # section_data_label = self.get_section_data('billing_email_field_label')
    ...
    def test_field_placeholder(self, logger):
        ...
        section_data_placeholder = self.get_section_data('billing_email_field_placeholder')
    ...
    def test_field_default_value(self, logger):
        ...
        section_data_default_value = self.get_section_data('billing_email_field_default_value')
    ...
    def test_field_class(self, logger):
        ...
        section_data_field_class = self.get_section_data('billing_email_field_class')
    ...
    def test_email_validation(self, logger):
        ...
        section_data_email_val = self.get_section_data('billing_email_email_validation')
    ...
    def test_required(self, logger):
        ...
        section_data_required = self.get_section_data('billing_email_required')
    ...
    def test_enable_field(self, logger):
        ...
        section_data_enable_field = self.get_section_data('billing_email_enable_field')
    ...
    def test_edit_field(self, logger):
        ...
        section_data_label = self.get_section_data('billing_email_edit_field')
        for label in section_data_label:
            first_name = 'jijo'
            company = 'zenn'
            ...
            phone = "9846789876"
            field_label = self.obj_base_page.strip_data(label['custom_field_label'])
            email = self.obj_base_page.strip_data(label['custom_field_input_value'])
            ...
            if (res_checkout_form_designer):
                ...
                if (result_checkout):
                    ...
                    result_thank_you, message = obj_thank_you_page.edit_field(email, self.section_type, self.action_type)
                    if (result_thank_you):
                        ...
                        result_edit_billing_address, message = obj_edit_billing_address.check_edit_billing_address(self.field_name, email)
                        if (result_edit_billing_address):
                            ...
                            result_admin_order, message_five = obj_admin_order_page.edit_field(email, self.field_name, self.section_type, self.action_type)
