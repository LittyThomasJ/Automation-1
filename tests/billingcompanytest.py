...
class BillingCompany(unittest.TestCase, BaseTest):
    """ For testing all tests within billing company testcases
    Calls to all the actions in each pages are done here
    """
    def __init__(self):
        self.row = 3
        self.field_name = 'billing_company'
    ...
    def test_field_label(self, logger):
        self.field_label('billing_company_field_label', logger, section_type=self.section_type, action_type=self.action_type, row=self.row, field_name=self.field_name)

        # section_data_label = self.get_section_data('billing_company_field_label')
    ...
    def test_field_placeholder(self, logger):
        section_data_placeholder = self.get_section_data('billing_company_field_placeholder')
    ...
    def test_field_default_value(self, logger):
        section_data_default_value = self.get_section_data('billing_company_field_default_value')
    ...
    def test_field_class(self, logger):
        section_data_field_class = self.get_section_data('billing_company_field_class')
    ...
    def test_required(self, logger):
        section_data_required = self.get_section_data('billing_company_required')
    ...
    def test_enable_field(self, logger):
        section_data_enable_field = self.get_section_data('billing_company_enable_field')
        for enable_field in section_data_enable_field:

            case_type = enable_field['custom_custom_value_type']
    ...
    def test_edit_field(self, logger):
        section_data_label = self.get_section_data('billing_company_edit_field')
        for label in section_data_label:
            ...
            field_label = self.obj_base_page.strip_data(label['custom_field_label'])
            company = self.obj_base_page.strip_data(label['custom_field_input_value'])
            ...
            if (res_checkout_form_designer):
                ...
                if (result_checkout):
                    ...
                    result_thank_you, message = obj_thank_you_page.edit_field(company, self.section_type, self.action_type)
                    if (result_thank_you):
                        ...
                        result_edit_billing_address, message = obj_edit_billing_address.check_edit_billing_address(self.field_name, company)
                        if (result_edit_billing_address):
                            ...
                            result_admin_order, message_five = obj_admin_order_page.edit_field(company, self.field_name, self.section_type, self.action_type)
                ...
                result, status = "Failed", 5
            self.print_message(test_case_title, status,  [self.field_name, last_name, field_label], message, result, logger, case_id)

    @classmethod
    def tearDownClass(cls):
        print("Test for Billing Company Completed")
        cls.driver.close()
        cls.driver.quit()
