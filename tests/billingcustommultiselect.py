...
class BillingCustomMultiselect(unittest.TestCase, BaseTest):
    """ For testing all tests within billing custom multiselect testcases
    ...
    def test_field_name(self, logger):
        self.field_name('billing_custom_multiselect_field_name', logger)
        # section_data_name = self.get_section_data('billing_custom_multiselect_field_name')
    ...
    def test_field_label(self, logger):
        self.field_label('billing_custom_multiselect_field_label', logger, section_type=self.section_type, action_type=self.action_type)

        # section_data_label = self.get_section_data('billing_custom_multiselect_field_label')
    ...
    def test_field_placeholder(self, logger):
        ...
        section_data_placeholder = self.get_section_data('billing_custom_multiselect_field_placeholder')
        for placeholder in section_data_placeholder:
            ...
            if (res_checkout_form_designer):
                if case_type == "without fieldname":
                    ...
                    obj_checkout_page = CheckoutPage(self.driver)
                    self.get_url(os.environ.get('WCFEF_CHECKOUT_PAGE_URL'))
                    result_checkout, message = obj_checkout_page.check_placeholder(field_name, field_placeholder, case_type, self.section_type, field_type)
                    if (result_checkout):
                        result, status = "Passed", 1
                    else:
                        result, status = "Failed", 5
            ...
            self.print_message(test_case_title, status,
                                                                 [field_name, field_type, field_placeholder],
                                                                 message, result, logger, case_id)

    def test_field_default_value(self, logger):

        section_data_default_value = self.get_section_data('billing_custom_multiselect_field_default_value')
        for default_values in section_data_default_value:
            ...
            obj_checkout_form_designer = CheckoutFormDesigner(self.driver)
            self.get_url(os.environ.get('WCFEF_LOGIN_URL'))
            res_checkout_form_designer, message = obj_checkout_form_designer.add_default_value(default_value, self.action_type, case_type, self.section_type, field_type=field_type, field_name=field_name, option_value_one=option_value_one, option_text_one=option_text_one, option_value_two=option_value_two,
            option_text_two=option_text_two)
            if (res_checkout_form_designer):
    ...
    def test_field_class(self, logger):
        ...
        section_data_field_class = self.get_section_data('billing_custom_multiselect_field_class')
        for field_class in section_data_field_class:
            field_type = self.obj_base_page.clean_data(field_class['custom_field_type'])
            field_name = self.obj_base_page.clean_data(field_class['custom_field_name'])
    ...
    def test_required(self, logger):
        ...
        section_data_required = self.get_section_data('billing_custom_multiselect_required')
        for required in section_data_required:
            ...
            obj_checkout_form_designer = CheckoutFormDesigner(self.driver)
            self.get_url(os.environ.get('WCFEF_LOGIN_URL'))
            res_checkout_form_designer, message = obj_checkout_form_designer.add_required(field_name, field_label, test_case_type, self.section_type, self.action_type, field_type=field_type, option_value_one=option_value_one, option_text_one=option_text_one, option_value_two=option_value_two, option_text_two=option_text_two, option_value_three=option_value_three, option_text_three=option_text_three)
            obj_product_page = ProductPage(self.driver)
            self.get_url(os.environ.get('WCFEF_PRODUCT_PAGE_URL'))
            obj_product_page.add_to_cart()
            if (res_checkout_form_designer):
    ...
    def test_enable_field(self, logger):
        ...
        section_data_enable_field = self.get_section_data('billing_custom_multiselect_enable_field')
    ...
    def test_display_order(self, logger):
        ...
        section_data_display_order = self.get_section_data('billing_custom_multiselect_display_order')
    ...
    def test_create_field(self, logger):
        ...
        section_data_label = self.get_section_data('billing_custom_multiselect_create_field')
        for label in section_data_label:
    ...
    def tearDownClass(cls):
        print("Custom multiselect Completed")
        cls.driver.close()
        cls.driver.quit()
