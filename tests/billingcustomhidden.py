...
class BillingCustomHidden(unittest.TestCase, BaseTest):
    """ For testing all tests within billing custom hidden testcases
    ...
    def test_field_label(self, logger):
        ...
        section_data_label = self.get_section_data('billing_custom_hidden_field_label')
        for label in section_data_label:
            default_value = self.obj_base_page.strip_data(label['custom_field_default_value'])
            ...
            res_checkout_form_designer, message  = obj_checkout_form_designer.add_label(field_label, case_type,
                                                                                           ...
                                                                                           field_name=field_name, field_default_value=default_value)
            ...
            if (res_checkout_form_designer):
                ...
                if (result_checkout):
                    ...
                    result_thank_you, message = obj_thank_you_page.check_label(field_label)
                    if result_thank_you:
                ...
                result, status = "Failed", 5
            self.print_message(test_case_title, status,  [field_name, field_label, field_type], message, result, logger, case_id)

    def test_field_name(self, logger):

        section_data_name = self.get_section_data('billing_custom_hidden_field_name')
    ...
    def test_field_default_value(self, logger):
        ...
        section_data_default_value = self.get_section_data('billing_custom_hidden_field_default_value')
        for default_values in section_data_default_value:
            ...
            field_label = self.obj_base_page.strip_data(default_values['custom_field_label'])
            case_id = default_values['id']
            test_case_title = default_values['title']
            obj_checkout_form_designer = CheckoutFormDesigner(self.driver)
            self.get_url(os.environ.get('WCFEF_LOGIN_URL'))
            res_checkout_form_designer, message = obj_checkout_form_designer.add_default_value(default_value, self.action_type, case_type, self.section_type, field_type=field_type, field_name=field_name, field_label=field_label)
            if (res_checkout_form_designer):
                ...
                if (result_checkout):
                    obj_thank_you_page = ThankYouPage(self.driver)
                    result_thank, message = obj_thank_you_page.check_default_value(default_value, case_type)
                    if (result_thank):
                        result, status = "Passed", 1
                    else:
                        result, status = "Failed", 5
    ...
    def test_field_class(self, logger):
        section_data_field_class = self.get_section_data('billing_custom_hidden_field_class')
        for field_class in section_data_field_class:
            ...
            if (res_checkout_form_designer):
                ...
                obj_checkout_page = CheckoutPage(self.driver)
                self.get_url(os.environ.get('WCFEF_CHECKOUT_PAGE_URL'))
                result_checkout, message = obj_checkout_page.check_field_class(field_name,
                                                                                   field_class_value, self.section_type, field_type=field_type)
                if (result_checkout):
                    result, status = "Passed", 1
                else:
                    result, status = "Failed", 5
    ...
    def test_enable_field(self, logger):
        ...
        section_data_enable_field = self.get_section_data('billing_custom_hidden_enable_field')
    ...
    def test_display_order(self, logger):
        ...
        section_data_field_class = self.get_section_data('billing_custom_hidden_display_order')
        for display_order in section_data_field_class:
            ...
            case_type = self.obj_base_page.strip_data(display_order['custom_custom_value_type'])
            field_default_value = self.obj_base_page.strip_data(display_order['custom_field_default_value'])
            ...
            obj_checkout_form_designer = CheckoutFormDesigner(self.driver)
            self.get_url(os.environ.get('WCFEF_LOGIN_URL'))
            res_checkout_form_designer, message = obj_checkout_form_designer.add_display_order(field_type, field_name, field_label, case_type, self.section_type, field_default_value=field_default_value)
            if (res_checkout_form_designer):
                # Check for checkout page for added field
                obj_checkout_page = CheckoutPage(self.driver)
                self.get_url(os.environ.get('WCFEF_CHECKOUT_PAGE_URL'))
                result_checkout, message = obj_checkout_page.place_order(field_name, self.section_type, field_type)
                if (result_checkout):
                    obj_thank_you_page = ThankYouPage(self.driver)
                    result_thank_you, message = obj_thank_you_page.check_order_page(field_label, field_default_value, case_type)
                    if (result_thank_you):
                        result, status = "Passed", 1
                    else:
                        result, status = "Failed", 5
            ...
            self.print_message(test_case_title, status,
                                                                 [field_name, field_type, field_default_value, field_label],
                                                                 message,
                                                                 result, logger, case_id)

    def test_create_field(self, logger):

        section_data_label = self.get_section_data('billing_custom_hidden_create_field')
        for label in section_data_label:
            ...
            field_name = self.obj_base_page.strip_data(label['custom_field_name'])
            field_label = self.obj_base_page.strip_data(label['custom_field_label'])
            field_default_value = self.obj_base_page.strip_data(label['custom_field_default_value'])
            case_type = 'valid'
            case_id = label['id']
            test_case_title = label['title']
            obj_product_page = ProductPage(self.driver)
            ...
            obj_checkout_form_designer = CheckoutFormDesigner(self.driver)
            self.get_url(os.environ.get('WCFEF_LOGIN_URL'))
            res_checkout_form_designer, message = obj_checkout_form_designer.add_label(field_label, case_type, self.section_type, self.action_type, field_type=field_type, field_name=field_name, field_default_value=field_default_value)
            if (res_checkout_form_designer):
                # Check for checkout page for added field
                obj_checkout_page = CheckoutPage(self.driver)
                self.get_url(os.environ.get('WCFEF_CHECKOUT_PAGE_URL'))
                result_checkout, message = obj_checkout_page.edit_field(field_name, field_label, self.section_type, first_name, last_name, company, country, street_address, apartment, town, state, post_code, phone, email)
                if (result_checkout):
                    # time.sleep(3)
                    obj_thank_you_page = ThankYouPage(self.driver)
                    result_thank_you, message = obj_thank_you_page.edit_field(field_default_value, self.section_type, self.action_type, field_label=field_label)
                    if (result_thank_you):
                        obj_admin_order_page = AdminOrderPage(self.driver)
                        self.get_url(os.environ.get('WCFEF_WOOCOMMERCE_ORDERS_URL'))
                        result_admin_order, message = obj_admin_order_page.edit_field(field_default_value, field_name, self.section_type, self.action_type, field_label=field_label)
                ...
                result, status = "Failed", 5
            self.print_message(test_case_title, status,
                                                                 [field_default_value, field_label, field_name, field_type],
                                                                 message,
                                                                 result, logger, case_id)
