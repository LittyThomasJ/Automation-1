...
class BillingCustomText(unittest.TestCase, BaseTest):
    """ For testing all tests within billing custom text testcases
    ...
    def test_field_name(self, logger):
        self.field_name('billing_custom_text_field_name', logger)
        # section_data_name = self.get_section_data('billing_custom_text_field_name')
        ...
        #     res_checkout_form_designer, message = obj_checkout_form_designer.add_name(field_name, field_type, case_type, self.section_type)
    ...
    def test_field_label(self, logger):
        self.field_label('billing_custom_text_field_label', logger, section_type=self.section_type, action_type=self.action_type)

        # section_data_label = self.get_section_data('billing_custom_text_field_label')
        ...
        #             obj_checkout_page = CheckoutPage(self.driver)
        #             self.get_url(os.environ.get('WCFEF_CHECKOUT_PAGE_URL'))
        #             result_checkout, message = obj_checkout_page.check_label(field_label, field_name, case_type, self.section_type, self.action_type)
        #             if (result_checkout):
        #                 result, status = "Passed", 1
        #             else:
        #                 result, status = "Failed", 5
    ...
    def test_field_placeholder(self, logger):
        ...
        section_data_placeholder = self.get_section_data('billing_custom_text_field_placeholder')
        for placeholder in section_data_placeholder:
            ...
            field_type = self.obj_base_page.strip_data(placeholder['custom_field_type'])
            field_name = self.obj_base_page.strip_data(placeholder['custom_field_name'])
            case_id = placeholder['id']
            test_case_title = placeholder['title']
            obj_checkout_form_designer = CheckoutFormDesigner(self.driver)
            self.get_url(os.environ.get('WCFEF_LOGIN_URL'))
            res_checkout_form_designer, message = obj_checkout_form_designer.add_placeholder(field_placeholder, case_type, self.section_type, self.action_type, field_type=field_type, field_name=field_name)
            if (res_checkout_form_designer):
                if case_type == "without fieldname":
                    result, status = "Passed", 1
                else:
                    ...
                    obj_checkout_page = CheckoutPage(self.driver)
                    self.get_url(os.environ.get('WCFEF_CHECKOUT_PAGE_URL'))
                    result_checkout, message = obj_checkout_page.check_placeholder(field_name, field_placeholder, case_type, self.section_type)
                    if (result_checkout):
                        result, status = "Passed", 1
                    else:
                        result, status = "Failed", 5
                ...
                result, status = "Failed", 5
            self.print_message(test_case_title, status,  [field_name, field_type, field_placeholder],
                                                           message, result, logger, case_id)

    def test_field_default_value(self, logger):

        section_data_default_value = self.get_section_data('billing_custom_text_field_default_value')
        for default_values in section_data_default_value:
            ...
            obj_checkout_form_designer = CheckoutFormDesigner(self.driver)
            self.get_url(os.environ.get('WCFEF_LOGIN_URL'))
            res_checkout_form_designer, message = obj_checkout_form_designer.add_default_value(default_value, self.action_type, case_type, self.section_type, field_type=field_type, field_name=field_name)
            if (res_checkout_form_designer):
                if case_type == "without fieldname":
                    result, status = "Passed", 1
                else:
    ...
    def test_field_class(self, logger):
        ...
        section_data_field_class = self.get_section_data('billing_custom_text_field_class')
    ...
    def test_phone_validation(self, logger):
        ...
        section_data_phone_val = self.get_section_data('billing_custom_text_phone_validation')
    ...
    def test_email_validation(self, logger):
        ...
        section_data_email_val = self.get_section_data('billing_custom_text_email_validation')
        for email_val in section_data_email_val:
            field_type = self.obj_base_page.clean_data(email_val['custom_field_type'])
            field_name = self.obj_base_page.strip_data(email_val['custom_field_name'])
    ...
    def test_post_code_validation(self, logger):
        ...
        section_data_post_code_val = self.get_section_data('billing_custom_text_postcode_validation')
    ...
    def test_number_validation(self, logger):
        ...
        section_data_number_val = self.get_section_data('billing_custom_text_number_validation')
    ...
    def test_state_validation(self, logger):
        ...
        section_data_state_val = self.get_section_data('billing_custom_text_state_validation')
    ...
    def test_enable_field(self, logger):
        ...
        section_data_enable_field = self.get_section_data('billing_custom_text_enable_field')
        for enable_field in section_data_enable_field:
            ...
            obj_checkout_form_designer = CheckoutFormDesigner(self.driver)
            self.get_url(os.environ.get('WCFEF_LOGIN_URL'))
            res_checkout_form_designer, message = obj_checkout_form_designer.add_field_enable(case_type, self.section_type, self.action_type, field_type=field_type, field_name=field_name)
            if (res_checkout_form_designer):
                # Check for checkout page for added field
                obj_checkout_page = CheckoutPage(self.driver)
                self.get_url(os.environ.get('WCFEF_CHECKOUT_PAGE_URL'))
    ...
    def test_display_order(self, logger):
        ...
        section_data_display_order = self.get_section_data('billing_custom_text_display_order')
        for display_order in section_data_display_order:
            ...
            obj_checkout_form_designer = CheckoutFormDesigner(self.driver)
            self.get_url(os.environ.get('WCFEF_LOGIN_URL'))
            res_checkout_form_designer, message = obj_checkout_form_designer.add_display_order(field_type, field_name, field_label, case_type, self.section_type)
            if (res_checkout_form_designer):
                # Check for checkout page for added field
                obj_checkout_page = CheckoutPage(self.driver)
                self.get_url(os.environ.get('WCFEF_CHECKOUT_PAGE_URL'))
                ...
                result, status = "Failed", 5
            self.print_message(test_case_title, status,  [field_name, field_type, field_input, field_label],
                                                           message,
                                                           result, logger, case_id)

    def test_required(self, logger):

        section_data_required = self.get_section_data('billing_custom_text_required')
        for required in section_data_required:
            field_type = self.obj_base_page.clean_data(required['custom_field_type'])
            field_name = self.obj_base_page.clean_data(required['custom_field_name'])
            ...
            obj_checkout_form_designer = CheckoutFormDesigner(self.driver)
            self.get_url(os.environ.get('WCFEF_LOGIN_URL'))
            res_checkout_form_designer, message = obj_checkout_form_designer.add_required(field_name, field_label, test_case_type, self.section_type, self.action_type, field_type=field_type)
            obj_product_page = ProductPage(self.driver)
            self.get_url(os.environ.get('WCFEF_PRODUCT_PAGE_URL'))
            obj_product_page.add_to_cart()
            if (res_checkout_form_designer):
                ...
                obj_checkout_page = CheckoutPage(self.driver)
                self.get_url(os.environ.get('WCFEF_CHECKOUT_PAGE_URL'))
                result_checkout, message = obj_checkout_page.check_required(field_name, self.section_type, test_case_type, self.action_type, field_input=field_input)
                if (result_checkout):
                    result, status = "Passed", 1
                else:
                    result, status = "Failed", 5
                ...
                result, status = "Failed", 5
            self.print_message(test_case_title, status,  [field_name, field_label], message,
                                                           result, logger, case_id)

    def test_create_field(self, logger):

        section_data_label = self.get_section_data('billing_custom_text_create_field')
        for label in section_data_label:
