...
class BillingState(unittest.TestCase, BaseTest):
    """ For testing all tests within billing state testcases
    Calls to all the actions in each pages are done here
    """
    def __init__(self):
        self.row = 8
        self.section_type = 'billing'
        self.field_name = 'billing_state'
    ...
    def test_field_label(self, logger):
        self.field_label('billing_state_field_label', logger, section_type=self.section_type, action_type=self.action_type, row=self.row, field_name=self.field_name)

        # section_data_label = self.get_section_data('billing_state_field_label')
    ...
    def test_field_placeholder(self, logger):
        section_data_placeholder = self.get_section_data('billing_state_field_placeholder')
    ...
    def test_field_default_value(self, logger):
        ...
        section_data_default_value = self.get_section_data('billing_state_field_default_value')
    ...
    def test_enable_field(self, logger):
        ...
        section_data_enable_field = self.get_section_data('billing_state_enable_field')
    ...
    def test_field_class(self, logger):
        section_data_field_class = self.get_section_data('billing_state_field_class')
    ...
    def test_required(self, logger):
        section_data_required = self.get_section_data('billing_state_required')
    ...
    def test_state_validation(self, logger):
        ...
        section_data_state_val = self.get_section_data('billing_state_state_validation')
        for state_val in section_data_state_val:
            ...
            if (res_checkout_form_designer):
                ...
                obj_checkout_page = CheckoutPage(self.driver)
                self.get_url(os.environ.get('WCFEF_CHECKOUT_PAGE_URL'))
                result_checkout, message = obj_checkout_page.check_field_validation(self.field_name, field_input, case_type, validation_type, self.section_type, country=billing_country, postcode=postcode)
                if (result_checkout):
                    result, status = "Passed", 1
                else:
                    result, status = "Failed", 5
                ...
                result, status = "Failed", 5
            self.print_message(test_case_title, status,  [self.field_name, postcode, field_input, billing_country, billing_state], message, result, logger, case_id)

    def test_edit_field(self, logger):
        section_data_label = self.get_section_data('billing_state_edit_field')
        for label in section_data_label:
