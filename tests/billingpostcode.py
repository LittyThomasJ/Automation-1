...
class BillingPostCode(unittest.TestCase, BaseTest):
    """ For testing all tests within billing post code testcases
    Calls to all the actions in each pages are done here
    """
    def __init__(self):
        self.row = 9
        self.field_name = 'billing_postcode'
    ...
    def test_field_label(self, logger):
        self.field_label('billing_postcode_field_label', logger, section_type=self.section_type, action_type=self.action_type, row=self.row, field_name=self.field_name)

        # section_data_label = self.get_section_data('billing_postcode_field_label')
    ...
    def test_field_placeholder(self, logger):
        section_data_placeholder = self.get_section_data('billing_postcode_field_placeholder')
    ...
    def test_field_default_value(self, logger):
        ...
        section_data_default_value = self.get_section_data('billing_postcode_field_default_value')
    ...
    def test_field_class(self, logger):
        section_data_field_class = self.get_section_data('billing_postcode_field_class')
    ...
    def test_post_code_validation(self, logger):
        ...
        section_data_post_code_val = self.get_section_data('billing_postcode_postcode_validation')
        for post_code_val in section_data_post_code_val:
            ...
            if (res_checkout_form_designer):
                ...
                result, status = "Failed", 5
            self.print_message(test_case_title, status,  [self.field_name, billing_country, billing_state, field_input], message, result, logger, case_id)

    def test_number_validation(self, logger):

        section_data_field_class = self.get_section_data('billing_postcode_number_validation')
        for field_class in section_data_field_class:
            field_input = self.obj_base_page.strip_data(field_class['custom_field_input_value'])
    ...
    def test_required(self, logger):
        ...
        section_data_required = self.get_section_data('billing_postcode_required')
    ...
    def test_enable_field(self, logger):
        ...
        section_data_enable_field = self.get_section_data('billing_postcode_enable_field')
    ...
    def test_edit_field(self, logger):
        ...
        section_data_label = self.get_section_data('billing_postcode_edit_field')
        for label in section_data_label:
            first_name = 'jijo'
            company = 'zenn'
    ...
    def tearDownClass(cls):
        print("Billing Postcode Completed")
        cls.driver.close()
        cls.driver.quit()
