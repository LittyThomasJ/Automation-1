...
class CustomUrl(unittest.TestCase, BaseTest):
    """ For testing all tests within billing custom week testcases
    ...
    def test_field_name(self, logger):
        self.field_name('custom_url_field_name', logger)
        # section_data_name = self.get_section_data('custom_url_field_name')
    ...
    def test_field_label(self, logger):
        self.field_label('custom_url_field_label', logger, section_type=self.section_type, action_type=self.action_type)

        # section_data_label = self.get_section_data('custom_url_field_label')
    ...
    def test_field_default_value(self, logger):
        ...
        section_data_default_value = self.get_section_data('custom_url_field_default_value')
    ...
    def test_field_placeholder(self, logger):
        ...
        section_data_placeholder = self.get_section_data('custom_url_field_placeholder')
    ...
    def test_field_class(self, logger):
        ...
        section_data_field_class = self.get_section_data('custom_url_field_class')
    ...
    def test_required(self, logger):
        ...
        section_data_required = self.get_section_data('custom_url_required')
        for required in section_data_required:
            ...
            if (res_checkout_form_designer):
                ...
                obj_checkout_page = CheckoutPage(self.driver)
                self.get_url(os.environ.get('WCFEF_CHECKOUT_PAGE_URL'))
                result_checkout, message = obj_checkout_page.check_required(field_name, self.section_type, test_case_type, self.action_type, field_input=field_input, field_type=field_type)
                if (result_checkout):
                    result, status = "Passed", 1
                else:
                    result, status = "Failed", 5
    ...
    def test_enable_field(self, logger):
        ...
        section_data_enable_field = self.get_section_data('custom_url_enable_field')
    ...
    def test_display_order(self, logger):
        ...
        section_data_display_order = self.get_section_data('custom_url_display_order')
    ...
    def test_create_field(self, logger):
        ...
        section_data_label = self.get_section_data('custom_url_create_field')
        for label in section_data_label:
