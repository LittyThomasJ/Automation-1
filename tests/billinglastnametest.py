...
class BillingLastName(unittest.TestCase, BaseTest):
    """ For testing all tests within billing last name testcases
    Calls to all the actions in each pages are done here
    """
    def __init__(self):
        self.row = 2
        self.field_name = 'billing_last_name'
    ...
    def test_field_label(self, logger):
        self.field_label('billing_last_name_field_label', logger, section_type=self.section_type, action_type=self.action_type, row=self.row, field_name=self.field_name)

        # section_data_label = self.get_section_data('billing_last_name_field_label')
    ...
    def test_field_placeholder(self, logger):
        ...
        section_data_placeholder = self.get_section_data('billing_last_name_field_placeholder')
    ...
    def test_field_default_value(self, logger):
        ...
        section_data_default_value = self.get_section_data('billing_last_name_field_default_value')
    ...
    def test_field_class(self, logger):
        ...
        section_data_field_class = self.get_section_data('billing_last_name_field_class')
    ...
    def test_required(self, logger):
        section_data_required = self.get_section_data('billing_last_name_required')
    ...
    def test_enable_field(self, logger):
        section_data_enable_field = self.get_section_data('billing_last_name_enable_field')
    ...
    def test_edit_field(self, logger):
        section_data_label = self.get_section_data('billing_last_name_edit_field')
        for label in section_data_label:
            ...
            field_label = self.obj_base_page.strip_data(label['custom_field_label'])
            last_name = self.obj_base_page.strip_data(label['custom_field_input_value'])
            case_type = self.obj_base_page.clean_data(label['custom_custom_value_type'])
            case_id = label['id']
            test_case_title = label['title']
            ...
            if (res_checkout_form_designer):
                ...
                if (result_checkout):
                    ...
                    result_thank_you, message = obj_thank_you_page.edit_field(last_name, self.section_type, self.action_type)
                    if (result_thank_you):
                        ...
                        result_edit_billing_address, message = obj_edit_billing_address.check_edit_billing_address(self.field_name, last_name)
                        if (result_edit_billing_address):
                            ...
                            result_admin_order, message_five = obj_admin_order_page.edit_field(last_name, self.field_name, self.section_type, self.action_type)
    ...
    def tearDownClass(cls):
        print("Test for Billing Last name Completed")
        cls.driver.close()
        cls.driver.quit()
