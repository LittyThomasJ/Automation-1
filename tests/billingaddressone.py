...
class BillingAddressOne(unittest.TestCase, BaseTest):
    """ For testing all tests within billing address one testcases
    ...
    def test_field_label(self, logger):
        self.field_label('billing_address_one_field_label', logger, section_type=self.section_type, action_type=self.action_type, row=self.row, field_name=self.field_name)
        # section_data_label = self.get_section_data('billing_address_one_field_label')
    ...
    def test_field_placeholder(self, logger):
        section_data_placeholder = self.get_section_data('billing_address_one_field_placeholder')
    ...
    def test_field_default_value(self, logger):
        section_data_default_value = self.get_section_data('billing_address_one_field_default_value')
    ...
    def test_field_class(self, logger):
        section_data_field_class = self.get_section_data('billing_address_one_field_class')
    ...
    def test_required(self, logger):
        section_data_required = self.get_section_data('billing_address_one_required')
    ...
    def test_enable_field(self, logger):
        section_data_enable_field = self.get_section_data('billing_address_one_enable_field')
    ...
    def test_edit_field(self, logger):
        section_data_label = self.get_section_data('billing_address_one_edit_field')
        for label in section_data_label:
            ...
            if (res_checkout_form_designer):
                ...
                if (result_checkout):
                    ...
                    if (result_thank_you):
                        ...
                        if (result_edit_billing_address):
                            ...
                            result_admin_order, message_five = obj_admin_order_page.edit_field(street_address, self.field_name, self.section_type, self.action_type)
                ...
                result, status = "Failed", 5
            self.print_message(test_case_title, status,  [street_address, field_label], message, result, logger, case_id)
    @classmethod
    def tearDownClass(cls):
        print("Billing Address One Completed")
        cls.driver.close()
        cls.driver.quit()
