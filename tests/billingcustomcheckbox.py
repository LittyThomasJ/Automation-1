...
class BillingCustomCheckbox(unittest.TestCase, BaseTest):
    """ For testing all tests within billing custom checkbox testcases
    ...
    def test_field_name(self, logger):
        self.field_name('billing_custom_checkbox_field_name', logger)
        # section_data_name = self.get_section_data('billing_custom_checkbox_field_name')
    ...
    def test_field_label(self, logger):
        ...
        # section_data_label = self.get_section_data('billing_custom_checkbox_field_label')
    ...
    def test_field_class(self, logger):
        ...
        section_data_field_class = self.get_section_data('billing_custom_checkbox_field_class')
    ...
    def test_required(self, logger):
        ...
        section_data_required = self.get_section_data('billing_custom_checkbox_required')
        for required in section_data_required:
            ...
            if (res_checkout_form_designer):
                ...
                obj_checkout_page = CheckoutPage(self.driver)
                self.get_url(os.environ.get('WCFEF_CHECKOUT_PAGE_URL'))
                result_checkout, message = obj_checkout_page.check_required(field_name, self.section_type, test_case_type, self.action_type, field_type=field_type)
                if (result_checkout):
                    result, status = "Passed", 1
                else:
                    result, status = "Failed", 5
    ...
    def test_enable_field(self, logger):
        ...
        section_data_enable_field = self.get_section_data('billing_custom_checkbox_enable_field')
    ...
    def test_display_order(self, logger):
        ...
        section_data_display_order = self.get_section_data('billing_custom_checkbox_display_order')
        for display_order in section_data_display_order:
            ...
            if (res_checkout_form_designer):
                ...
                if (result_checkout):
                    ...
                    result_thank_you, message = obj_thank_you_page.check_order_page(field_label, "1", case_type)
                    if (result_thank_you):
                        result, status = "Passed", 1
                    else:
                        result, status = "Failed", 5
    ...
    def test_create_field(self, logger):
        ...
        section_data_label = self.get_section_data('billing_custom_checkbox_create_field')
        for label in section_data_label:
            ...
            if (res_checkout_form_designer):
                ...
                if (result_checkout):
                    # time.sleep(3)
                    obj_thank_you_page = ThankYouPage(self.driver)
                    result_thank_you, message = obj_thank_you_page.edit_field("1", self.section_type, self.action_type, field_label=field_label)
                    if (result_thank_you):
                        obj_admin_order_page = AdminOrderPage(self.driver)
                        self.get_url(os.environ.get('WCFEF_WOOCOMMERCE_ORDERS_URL'))
                        result_admin_order, message = obj_admin_order_page.edit_field("1", field_name, self.section_type, self.action_type, field_label=field_label)
                ...
                result, status = "Failed", 5
            self.print_message(test_case_title, status, [field_input, field_label, field_name, field_type],
                                                                 message,
                                                                 result, logger, case_id)

    @classmethod
    def tearDownClass(cls):
        print("Custom Checkbox Completed")
        cls.driver.close()
        cls.driver.quit()
