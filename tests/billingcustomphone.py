...
class BillingCustomPhone(unittest.TestCase, BaseTest):
    """ For testing all tests within billing custom phone testcases
    ...
    def test_field_name(self, logger):
        self.field_name('billing_custom_phone_field_name', logger)
        # section_data_name = self.get_section_data('billing_custom_phone_field_name')
        # for name in section_data_name:
        #     field_type =self.obj_base_page.strip_data( name['custom_field_type'])
    ...
    def test_field_label(self, logger):
        self.field_label('billing_custom_phone_field_label', logger, section_type=self.section_type, action_type=self.action_type)

        # section_data_label = self.get_section_data('billing_custom_phone_field_label')
    ...
    def test_field_placeholder(self, logger):
        ...
        section_data_placeholder = self.get_section_data('billing_custom_phone_field_placeholder')
        for placeholder in section_data_placeholder:
            ...
            obj_checkout_form_designer = CheckoutFormDesigner(self.driver)
            self.get_url(os.environ.get('WCFEF_LOGIN_URL'))
            res_checkout_form_designer, message = obj_checkout_form_designer.add_placeholder(field_placeholder,
                                                                                                 case_type,
                                                                                                 self.section_type,
                                                                                                 self.action_type,
                                                                                                 field_type=field_type,
            ...
            if (res_checkout_form_designer):
                if case_type == "without fieldname":
                    ...
                    obj_checkout_page = CheckoutPage(self.driver)
                    self.get_url(os.environ.get('WCFEF_CHECKOUT_PAGE_URL'))
                    result_checkout, message = obj_checkout_page.check_placeholder(field_name, field_placeholder,
                                                                                       case_type, self.section_type)
                    if (result_checkout):
                        result, status = "Passed", 1
                    else:
    ...
    def test_field_default_value(self, logger):
        ...
        section_data_default_value = self.get_section_data('billing_custom_phone_field_default_value')
        for default_values in section_data_default_value:
            ...
            obj_checkout_form_designer = CheckoutFormDesigner(self.driver)
            self.get_url(os.environ.get('WCFEF_LOGIN_URL'))
            res_checkout_form_designer, message = obj_checkout_form_designer.add_default_value(default_value,
                                                                                                   self.action_type,
                                                                                                   case_type,
                                                                                                   self.section_type,
                                                                                                   field_type=field_type,
            ...
            if (res_checkout_form_designer):
                if case_type == "without fieldname":
                    ...
                    obj_checkout_page = CheckoutPage(self.driver)
                    self.get_url(os.environ.get('WCFEF_CHECKOUT_PAGE_URL'))
                    result_checkout, message = obj_checkout_page.check_default_value(field_name, default_value,
                                                                                         case_type, self.section_type,
                                                                                         self.action_type,
                                                                                         field_type=field_type)
                    if (result_checkout):
    ...
    def test_field_class(self, logger):
        ...
        section_data_field_class = self.get_section_data('billing_custom_phone_field_class')
    ...
    def test_phone_validation(self, logger):
        ...
        section_data_phone_val = self.get_section_data('billing_custom_phone_phone_validation')
        for phone_val in section_data_phone_val:
            ...
            obj_checkout_form_designer = CheckoutFormDesigner(self.driver)
            self.get_url(os.environ.get('WCFEF_LOGIN_URL'))
            res_checkout_form_designer, message = obj_checkout_form_designer.add_field_validation(validation_type,
                                                                                                      self.section_type,
                                                                                                      field_name,
                                                                                                      self.action_type,
                                                                                                      field_type=field_type)
            if (res_checkout_form_designer):
                ...
                obj_checkout_page = CheckoutPage(self.driver)
                self.get_url(os.environ.get('WCFEF_CHECKOUT_PAGE_URL'))
                result_checkout, message = obj_checkout_page.check_field_validation(field_name, field_input,
                                                                                        case_type, validation_type,
                                                                                        self.section_type)
                if (result_checkout):
                    result, status = "Passed", 1
                else:
    ...
    def test_number_validation(self, logger):
        ...
        section_data_number_val = self.get_section_data('billing_custom_phone_number_validation')
    ...
    def test_required(self, logger):
        ...
        section_data_required = self.get_section_data('billing_custom_phone_required')
        for required in section_data_required:
            ...
            obj_checkout_form_designer = CheckoutFormDesigner(self.driver)
            self.get_url(os.environ.get('WCFEF_LOGIN_URL'))
            res_checkout_form_designer, message = obj_checkout_form_designer.add_required(field_name, field_label,
                                                                                              test_case_type,
                                                                                              self.section_type,
                                                                                              self.action_type,
                                                                                              field_type=field_type)
            ...
            if (res_checkout_form_designer):
                ...
                obj_checkout_page = CheckoutPage(self.driver)
                self.get_url(os.environ.get('WCFEF_CHECKOUT_PAGE_URL'))
                result_checkout, message = obj_checkout_page.check_required(field_name, self.section_type,
                                                                                test_case_type, self.action_type,
    ...
    def test_enable_field(self, logger):
        ...
        section_data_enable_field = self.get_section_data('billing_custom_phone_enable_field')
    ...
    def test_display_order(self, logger):
        ...
        section_data_display_order = self.get_section_data('billing_custom_phone_display_order')
        for display_order in section_data_display_order:
            ...
            obj_checkout_form_designer = CheckoutFormDesigner(self.driver)
            self.get_url(os.environ.get('WCFEF_LOGIN_URL'))
            res_checkout_form_designer, message = obj_checkout_form_designer.add_display_order(field_type,
                                                                                                   field_name,
                                                                                                   field_label,
                                                                                                   case_type,
                                                                                                   self.section_type)
            if (res_checkout_form_designer):
                ...
                if (result_checkout):
                    ...
                    result_thank_you, message = obj_thank_you_page.check_order_page(field_label, field_input,
                                                                                          case_type)
                    if (result_thank_you):
                        result, status = "Passed", 1
                    else:
    ...
    def test_create_field(self, logger):
        ...
        section_data_label = self.get_section_data('billing_custom_phone_create_field')
        for label in section_data_label:
