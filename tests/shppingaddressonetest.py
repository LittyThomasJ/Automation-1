...
class ShippingAddressOne(unittest.TestCase, BaseTest):
    """ For testing all tests within shipping address one testcases
    Calls to all the actions in each pages are done here
    """
    def __init__(self):
        self.row = 5
        self.field_name = 'shipping_address_1'
    ...
    def test_field_label(self, logger):
        self.field_label('shipping_address_one_field_label', logger, section_type=self.section_type, action_type=self.action_type, row=self.row, field_name=self.field_name)

        # section_data_label = self.get_section_data('shipping_address_one_field_label')
    ...
    def test_field_placeholder(self, logger):
        ...
        section_data_placeholder = self.get_section_data('shipping_address_one_field_placeholder')
    ...
    def test_field_default_value(self, logger):
        ...
        section_data_default_value = self.get_section_data('shipping_address_one_field_default_value')
    ...
    def test_field_class(self, logger):
        ...
        section_data_field_class = self.get_section_data('shipping_address_one_field_class')
    ...
    def test_required(self, logger):
        ...
        section_data_required = self.get_section_data('shipping_address_one_required')
    ...
    def test_enable_field(self, logger):
        ...
        section_data_enable_field = self.get_section_data('shipping_address_one_enable_field')
    ...
    def test_edit_field(self, logger):
        ...
        section_data_label = self.get_section_data('shipping_address_one_edit_field')
        for label in section_data_label:
            first_name = "phhhh"
            last_name = "jkkkjj"
            state = "KL"
            ...
            field_label = self.obj_base_page.strip_data(label['custom_field_label'])
            street_address = self.obj_base_page.strip_data(label['custom_field_input_value'])
            ...
            if (res_checkout_form_designer):
                ...
                if (result_checkout):
                    ...
                    result_thank_you, message = obj_thank_you_page.edit_field(street_address, self.section_type, self.action_type)
                    if (result_thank_you):
                        ...
                        result_edit_billing_address, message = obj_edit_billing_address.check_edit_billing_address(self.field_name, street_address)
                        if (result_edit_billing_address):
                            ...
                            result_admin_order, message = obj_admin_order_page.edit_field(street_address, self.field_name, self.section_type, self.action_type)
    ...
    def tearDownClass(cls):
        print("Test for Shipping Address One Completed")
        cls.driver.close()
        cls.driver.quit()
