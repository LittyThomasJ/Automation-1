...
class ShippingPostCode(unittest.TestCase, BaseTest):
    """ For testing all tests within shipping post code testcases
    Calls to all the actions in each pages are done here
    """
    def __init__(self):
        self.row = 9
        self.field_name = 'shipping_postcode'
    ...
    def test_field_label(self, logger):
        self.field_label('shipping_post_code_field_label', logger, section_type=self.section_type, action_type=self.action_type, row=self.row, field_name=self.field_name)

        # section_data_label = self.get_section_data('shipping_post_code_field_label')
    ...
    def test_field_placeholder(self, logger):
        ...
        section_data_placeholder = self.get_section_data('shipping_post_code_field_placeholder')
    ...
    def test_field_default_value(self, logger):
        ...
        section_data_default_value = self.get_section_data('shipping_post_code_field_default_value')
    ...
    def test_field_class(self, logger):
        ...
        section_data_field_class = self.get_section_data('shipping_post_code_field_class')
    ...
    def test_number_validation(self, logger):
        ...
        section_data_field_class = self.get_section_data('shipping_post_code_number_validation')
        for number_val in section_data_field_class:
    ...
    def test_required(self, logger):
        ...
        section_data_required = self.get_section_data('shipping_post_code_required')
    ...
    def test_post_code_validation(self, logger):
        ...
        section_data_field_class = self.get_section_data('shipping_post_code_postcode_validation')
        for post_code_val in section_data_field_class:
            ...
            if (res_checkout_form_designer):
                ...
                obj_checkout_page = CheckoutPage(self.driver)
                self.get_url(os.environ.get('WCFEF_CHECKOUT_PAGE_URL'))
                result_checkout, message = obj_checkout_page.check_field_validation(self.field_name, field_input, case_type, validation_type, self.section_type, country=billing_country, state=billing_state)
                if (result_checkout):
                    result, status = "Passed", 1
                else:
                    result, status = "Failed", 5
                ...
                result, status = "Failed", 5
            self.print_message(test_case_title, status,  [self.field_name, field_input, billing_country, billing_state],
                                                              message,
                                                              result, logger, case_id)

    def test_enable_field(self, logger):

        section_data_enable_field = self.get_section_data('shipping_post_code_enable_field')
    ...
    def test_edit_field(self, logger):
        ...
        section_data_label = self.get_section_data('shipping_post_code_edit_field')
        for label in section_data_label:
            ...
            field_label = self.obj_base_page.strip_data(label['custom_field_label'])
            post_code = self.obj_base_page.strip_data(label['custom_field_input_value'])
            case_type = self.obj_base_page.clean_data(label['custom_custom_value_type'])
            case_id = label['id']
            test_case_title = label['title']
            obj_checkout_form_designer = CheckoutFormDesigner(self.driver)
            ...
            if (res_checkout_form_designer):
                ...
                if (result_checkout):
                    ...
                    result_thank_you, message = obj_thank_you_page.edit_field(post_code, self.section_type, self.action_type)
                    if (result_thank_you):
                        ...
                        result_edit_billing_address, message = obj_edit_billing_address.check_edit_billing_address(self.field_name, post_code)
                        if (result_edit_billing_address):
                            ...
                            result_admin_order, message_five = obj_admin_order_page.edit_field(post_code, self.field_name, self.section_type, self.action_type)
                ...
                result, status = "Failed", 5
            self.print_message(test_case_title, status,  [post_code, field_label],
                                                          message,
                                                                  result, logger, case_id)

    @classmethod
    def tearDownClass(cls):
        print("Shipping Postcode Completed")
        cls.driver.close()
        cls.driver.quit()
