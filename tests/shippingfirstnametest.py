...
class ShippingFirstName(unittest.TestCase, BaseTest):
    """ For testing all tests within shipping first name testcases
    Calls to all the actions in each pages are done here
    """
    def __init__(self):
        self.row = 1
        self.field_name = 'shipping_first_name'
    ...
    def test_field_label(self, logger):
        self.field_label('shipping_first_name_field_label', logger, section_type=self.section_type, action_type=self.action_type, row=self.row, field_name=self.field_name)

        # section_data_label = self.get_section_data('shipping_first_name_field_label')
    ...
    def test_field_placeholder(self, logger):
        ...
        section_data_placeholder = self.get_section_data('shipping_first_name_field_placeholder')
    ...
    def test_field_default_value(self, logger):
        ...
        section_data_default_value = self.get_section_data('shipping_first_name_field_default_value')
        for default_values in section_data_default_value:
            ...
            case_type = self.obj_base_page.strip_data(default_values['custom_custom_value_type'])
    ...
    def test_field_class(self, logger):
        ...
        section_data_field_class = self.get_section_data('shipping_first_name_field_class')
    ...
    def test_required(self, logger):
        ...
        section_data_required = self.get_section_data('shipping_first_name_required')
    ...
    def test_enable_field(self, logger):
        ...
        section_data_enable_field = self.get_section_data('shipping_first_name_enable_field')
    ...
    def test_edit_field(self, logger):
        ...
        section_data_label = self.get_section_data('shipping_first_name_edit_field')
        for label in section_data_label:
            company = 'zenn'
            ...
            field_label = self.obj_base_page.strip_data(label['custom_field_label'])
            first_name = self.obj_base_page.strip_data(label['custom_field_input_value'])
            case_type = self.obj_base_page.clean_data(label['custom_custom_value_type'])
            case_id = label['id']
            test_case_title = label['title']
            obj_checkout_form_designer = CheckoutFormDesigner(self.driver)
            ...
            if (res_checkout_form_designer):
                ...
                if (result_checkout):
                    ...
                    result_thank_you, message = obj_thank_you_page.edit_field(first_name, self.section_type, self.action_type)
                    if (result_thank_you):
                        ...
                        result_edit_billing_address, message = obj_edit_billing_address.check_edit_billing_address(self.field_name, first_name)
                        if (result_edit_billing_address):
                            ...
                            result_admin_order, message_five = obj_admin_order_page.edit_field(first_name, self.field_name, self.section_type, self.action_type)
                ...
                result, status = "Failed", 5
            self.print_message(test_case_title, status,  [first_name, field_label], message, result, logger, case_id)

    @classmethod
    def tearDownClass(cls):
        print("Shipping First Name Completed")
        cls.driver.close()
        cls.driver.quit()
