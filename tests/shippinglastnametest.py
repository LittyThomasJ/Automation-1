...
class ShippingLastName(unittest.TestCase, BaseTest):
    """ For testing all tests within shipping last name testcases
    Calls to all the actions in each pages are done here
    """
    def __init__(self):
        self.row = 2
        self.field_name = 'shipping_last_name'
    ...
    def test_field_label(self, logger):
        self.field_label('shipping_last_name_field_label', logger, section_type=self.section_type, action_type=self.action_type, row=self.row, field_name=self.field_name)

        # section_data_label = self.get_section_data('shipping_last_name_field_label')
    ...
    def test_field_placeholder(self, logger):
        ...
        section_data_placeholder = self.get_section_data('shipping_last_name_field_placeholder')
    ...
    def test_field_default_value(self, logger):
        ...
        section_data_default_value = self.get_section_data('shipping_last_name_field_default_value')
    ...
    def test_field_class(self, logger):
        ...
        section_data_field_class = self.get_section_data('shipping_last_name_field_class')
    ...
    def test_enable_field(self, logger):
        ...
        section_data_enable_field = self.get_section_data('shipping_last_name_enable_field')
    ...
    def test_required(self, logger):
        ...
        section_data_required = self.get_section_data('shipping_last_name_required')
    ...
    def test_edit_field(self, logger):
        ...
        section_data_label = self.get_section_data('shipping_last_name_edit_field')
        for label in section_data_label:
            company = 'zenn'
            ...
            if (res_checkout_form_designer):
                ...
                result, status = "Failed", 5
            self.print_message(test_case_title, status,  [last_name, field_label], message, result, logger, case_id)

    @classmethod
    def tearDownClass(cls):
        print("Shipping Last name Completed")
        cls.driver.close()
        cls.driver.quit()
